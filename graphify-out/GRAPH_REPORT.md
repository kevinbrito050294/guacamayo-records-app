...
## Corpus Check
- 45 files · ~90,060 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 7 file(s) not represented in the graph (top: (none) 4, .csv 1, .exe 1)

## Summary
- 310 nodes · 435 edges · 34 communities (20 shown, 14 thin omitted)
- Extraction: 81% EXTRACTED · 18% INFERRED · 1% AMBIGUOUS · INFERRED: 79 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5150eaee`
...
- `Panel de administracion (4 herramientas)` --semantically_similar_to--> `Panel de administracion (4 secciones)`  [INFERRED] [semantically similar]
  GUIA_RAPIDA.md → MANUAL_OPERACIONES.md
...
Cohesion: 0.12
Nodes (31): DATOS_EJEMPLO.csv - 10 vinilos de prueba, Conjunto de documentacion (9 guias), GuacamayoRecords - e-commerce de vinilos (inventario), SCRIPT_MYSQL_BACKUP.sql - backup en formato MySQL, Estado: PRODUCTION READY v1.0, Componentes React + utilidades TypeScript (12), Esquema de base de datos Supabase (5 tablas + RLS), GuacamayoRecords - arquitectura tecnica (+23 more)

### Community 2 - "Row Level Security (RLS) + validaciones"
Cohesion: 0.09
Nodes (29): Tabla clientes, Tabla detalles_pedido (items de orden), Variables de entorno VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY, Flujos de usuario (compra, carga de vinilos, actualizar tasas), Importador CSV (BulkImporter), Tabla inventario_vinilos (catalogo/SKU/album/artista), Tabla pedidos (ordenes de compra), Row Level Security (RLS) + validaciones (+21 more)
...
Cohesion: 0.15
Nodes (17): Tabla configuracion_divisas (tasas DOLAR_BLUE/USDT), Conversion de divisas (USD a ARS/USDT), Gestor de tasas (CurrencyManager), Herramienta: Tasas de Cambio, Archivos y rutas importantes, Dolar Blue y tasas de cambio, Gestion de precios (USD a ARS/USDT), Mejores practicas (+9 more)
...
## Knowledge Gaps
- **144 isolated node(s):** `Divisa`, `Page`, `CSVRow`, `CurrencyManagerProps`, `AdminLoginProps` (+139 more)
...
- **What is the exact relationship between `Guacamayo Records Brand` and `Music Marketing Promotion Content`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `lucide-react` connect `App.tsx` to `AdminPanel.tsx`, `package.json`, `FilterPanel.tsx`?**
  _High betweenness centrality (0.141) - this node is a cross-community bridge._
- **Why does `react` connect `App.tsx` to `AdminPanel.tsx`, `package.json`, `FilterPanel.tsx`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `BulkImporter()` connect `Row Level Security (RLS) + validaciones` to `App.tsx`, `AdminPanel.tsx`?**
