...
interface CatalogProps {
  ...
  divisaActiva: 'USD' | 'ARS' | 'USDT'; // Agregamos la prop necesaria
}
  ...
  useEffect(() => {
    async function cargarPrecios() {
      try {
        ...
        // Convertimos los precios de todos los vinilos
        for (const v of vinilos) {
          try {
            const conversion = await convertirPrecio(v.precio_venta);
            nuevosPrecios[v.id] = conversion;
          } catch (err) {
            // Fallback en caso de error de conexión con la tabla de divisas
    ...
    // El efecto se dispara si cambian los vinilos O si el usuario cambia la moneda
      ...
      <div className="w-full">
        {vinilosFiltrados.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
            {vinilosFiltrados.map((vinilo) => (
              <VinylCard
                ...
                divisaActiva={divisaActiva} // Enviamos la divisa a la card
              />
            ))}
          </div>
        ) : (
          <div className="text-center py-20 md:py-32 bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800">
            ...
            <p className="text-slate-400 dark:text-slate-500 font-medium">No encontramos ese surco...</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-12 md:mt-20 p-6 md:p-8 bg-slate-900 dark:bg-amber-500 rounded-[2rem] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 transition-all duration-500">
        <div className="flex items-center gap-4 text-center md:text-left">
          ...
          <div>
            ...
            <p className="text-amber-200/60 dark:text-slate-900/60 text-[10px] font-bold uppercase tracking-widest mt-1">
        ...
        {cargandoPrecios ? (
          <div className="flex items-center gap-3 bg-white/10 px-4 py-2 rounded-full">
             ...
             <span className="text-[10px] font-black text-white dark:text-slate-900 uppercase">Calculando conversión...</span>
          </div>
        ) : (
          <div className="text-[10px] font-black text-emerald-400 dark:text-slate-900 border border-emerald-500/40 dark:border-slate-900/40 px-6 py-2 rounded-full uppercase tracking-widest bg-emerald-500/5">
